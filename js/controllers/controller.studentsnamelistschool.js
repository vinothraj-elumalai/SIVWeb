sivwebapp.controller('studentsNameListSchoolCtrl', function($scope, $http, currentHost, hosturl, constantService, Auth) {
    $scope.userdata = Auth.isLoggedIn();
    $scope.studentsnamelistschooldata = {};
    $scope.admissionplayschoolprint = {};
    $scope.studentsnamelistschooldata.instituteid = $scope.userdata.instituteid;
    $scope.studentsNameListSubmit = function(){
    	console.log($scope.studentsnamelistschooldata);
        $http({
                url: hosturl+"/api/v1/schoolstudentnamelist",
                method: "POST",
